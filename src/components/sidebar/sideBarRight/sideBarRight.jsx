...
const SideBarRight = ({admins, online, offline}) => {
    return (
        <div className={cl.onlineArea}>
            ...
            {
                admins.map((user) => <UserElem key={user.name} name={user.name} status={user.status}/>)
            }
            <h1>MEMBERS - {online.length}</h1>
            {
                online.map((user) => <UserElem key={user.name} name={user.name} status={user.status}/>)
            }
            <h1>OFFLINE - {offline.length}</h1>
            {
                offline.map((user) => <UserElem key={user.name} name={user.name} status={user.status}/>)
            }
        </div>
    );
};
